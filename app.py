...
def index():
    ...
    ids = db.execute(
        "SELECT user_loan_id FROM loans WHERE user_id = ?", user_id)

    banks = db.execute(
        "SELECT DISTINCT banks.bank FROM banks JOIN loans ON loans.bank_id = banks.id WHERE loans.user_id = ?", user_id)
    banks.sort(key=lambda x: x["bank"])

    frequencies = db.execute(
        "SELECT DISTINCT frequencies.frequency FROM frequencies JOIN loans ON loans.payment_frequency = frequencies.id where loans.user_id = ?", user_id)

    types = db.execute(
        "SELECT DISTINCT types.type FROM types JOIN loans ON loans.interest_rate_type = types.id WHERE loans.user_id = ?", user_id)
...
def amortizations():
    ...
    user_amortization_ids = db.execute(
        "SELECT user_amortization_id FROM amortizations WHERE user_id = ?", user_id)
    user_loan_ids = db.execute(
        "SELECT DISTINCT loans.user_loan_id FROM loans JOIN amortizations ON amortizations.loan_id = loans.id WHERE amortizations.user_id = ?", user_id)
...
# info for filters
    ids = db.execute(
        "SELECT user_bank_id FROM banks WHERE user_id = ?", user_id)
...
def add_loans():
    ...
    compounding_periods = db.execute(
        "SELECT * FROM frequencies WHERE frequency != 'At maturity'")
    ...
    # errors in form data
        ...
        bank = db.execute("SELECT * FROM banks WHERE user_id = ? AND bank = ?",
                          user_id, cleaned_data["bank"])[0]["id"]
        ...
        payment_frequency = db.execute(
            "SELECT * FROM frequencies WHERE frequency = ?", cleaned_data["payment_frequency"])[0]["id"]
        interest_rate_type = db.execute(
            "SELECT * FROM types WHERE type = ?", cleaned_data["interest_rate_type"])[0]["id"]
        nominal_rate_compounding_period = db.execute(
            "SELECT * FROM frequencies WHERE frequency = ?", cleaned_data["nominal_rate_compounding_period"])[0]["id"]
        interest_payment_frequency = db.execute(
            "SELECT * FROM frequencies WHERE frequency = ?", cleaned_data["interest_payment_frequency"])[0]["id"]

        db.execute("INSERT INTO loans(user_id, user_loan_id, face_value, principal, bank_id, issue_date, loan_term, maturity_date, interest_rate, payment_frequency, interest_rate_type, nominal_rate_compounding_period, interest_payment_frequency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   user_id, next_id,  face_value, principal, bank, issue_date, loan_term, maturity_date, interest_rate, payment_frequency, interest_rate_type, nominal_rate_compounding_period, interest_payment_frequency)
        return redirect("/")
...
def edit_loan():
    ...
    # user got here via post
        ...
        try:
            ...
            db.execute("UPDATE loans SET face_value = ?, principal = ?, bank_id = ?, issue_date = ?, loan_term = ?, maturity_date = ?, interest_rate = ?, payment_frequency = ?, interest_rate_type = ?, nominal_rate_compounding_period = ?, interest_payment_frequency = ? WHERE user_id = ? AND user_loan_id = ?",
                       face_value, principal, bank, issue_date, loan_term, maturity_date, interest_rate, payment_frequency, interest_rate_type, nominal_rate_compounding_period, interest_payment_frequency, user_id, user_loan_id)
        return redirect("/")
...
def delete_loan():
    ...
    if user_loan_id:
        ...
        try:
            loan_id = db.execute(
                "SELECT id FROM loans WHERE user_id = ? AND user_loan_id = ?", user_id, user_loan_id)[0]["id"]
            #  checking if loan is in use by some amortization
            amortizations = db.execute(
                "SELECT * FROM amortizations WHERE loan_id = ?", loan_id)
...
def amortize():
    ...
    if request.method == "GET":
        ...
        if user_loan_id:
            ...
            try:
                ...
                loan = db.execute(
                    "SELECT * FROM loans WHERE user_id = ? AND user_loan_id = ?", user_id, user_loan_id)[0]
    ...
    # errors in form data
        ...
        # save record
        # Getting data to save
        loan_id = db.execute("SELECT * FROM loans WHERE user_id = ? AND user_loan_id = ?",
                             user_id, cleaned_data["user_loan_id"])[0]["id"]
        amort_value = cleaned_data["amort_value"]
        amort_date = cleaned_data["amort_date"]

        db.execute("INSERT INTO amortizations(user_id, loan_id, user_amortization_id, amort_value, amort_date) VALUES (?, ?, ?, ?, ?)",
                   user_id, loan_id, next_id,  amort_value, amort_date)
        # Update loan's balance
        principal = db.execute("SELECT principal FROM loans WHERE id = ?", loan_id)[
            0]["principal"]
        updated_principal = principal - amort_value
        db.execute("UPDATE loans SET principal = ? WHERE id = ?",
                   updated_principal, loan_id)
...
def add_amortizations():
    ...
    name = user["username"]
    loans = db.execute(
        "SELECT * FROM loans WHERE user_id = ? AND principal != 0", user_id)
...
def edit_amortization():
    ...
    if request.method == "GET":
        ...
        if user_amortization_id:
            ...
            try:
                data = db.execute(
                    "SELECT * FROM amortizations WHERE user_id = ? AND user_amortization_id = ?", user_id, user_amortization_id)[0]
                loan_id = data["loan_id"]
                loan = db.execute(
                    "SELECT * FROM loans WHERE id = ?", loan_id)[0]
    ...
    # user got here via post
        ...
        # preventing user from editing info he does not owns
        try:
            errors, cleaned_data = Amortization.validate(
                data, user_id, db, "edit")
        except IndexError:
            pass
        else:
            # errors in form data
            ...
            amort_value = cleaned_data["amort_value"]
            amort_date = cleaned_data["amort_date"]
            db.execute("UPDATE amortizations SET amort_value = ?, amort_date = ? WHERE user_amortization_id = ? and user_id = ?",
                       amort_value, amort_date, user_amortization_id, user_id)
...
def delete_amortization():
    ...
    if user_amortization_id:
        try:
            ...
            db.execute("DELETE FROM amortizations WHERE id = ?",
                       amortization_id)
        except IndexError:
            pass
    return redirect("/amortizations")
...
def add_banks():
    ...
    # user got here via post
        ...
        db.execute(
            "INSERT INTO banks(user_id, user_bank_id, bank) VALUES (?, ?, ?)", user_id, next_id, bank)
...
def edit_bank():
    ...
    if request.method == "GET":
        ...
        if user_bank_id:
            ...
            try:
                data = db.execute(
                    "SELECT * FROM banks WHERE user_id = ? AND user_bank_id = ?", user_id, user_bank_id)[0]
            except IndexError:
                pass
            else:
    ...
    # user got here via post
        ...
        try:
            ...
            # update record
            bank = cleaned_data["bank"]
            db.execute("UPDATE banks SET bank = ? WHERE user_bank_id = ? and user_id = ?",
                       bank, user_bank_id, user_id)
        return redirect("/banks")
...
def delete_bank():
    ...
    if user_bank_id:
        try:
            # preventing user from deleting info he does not owns
            bank_id = db.execute(
                "SELECT id FROM banks WHERE user_id = ? AND user_bank_id = ?", user_id, user_bank_id)[0]["id"]
            #  checking if bank is in use by some loan
            loans = db.execute(
                "SELECT * FROM loans WHERE bank_id = ?", bank_id)
            if len(loans) != 0:
                message = "Can not delete bank because it is been used by some loan."
                mtype = "error-"
                banks = db.execute(
                    "SELECT * FROM banks WHERE user_id = ?", user_id)
                # info for filters
                ids = db.execute(
                    "SELECT user_bank_id FROM banks WHERE user_id = ?", user_id)
...
def filter():
    ...
    if user_loan_id := data.get("user_loan_id"):
        ...
        try:
            bank_id = db.execute(
                "SELECT * FROM banks WHERE bank = ? AND user_id = ?", data.get("bank"), user_id)[0]["id"]
        except IndexError:
            bank_id = ""
            # Query database for loans to show
        loans = db.execute(f"""SELECT loans.id, 
        ...
        try:
            interest_rate_type = db.execute(
                "SELECT * FROM types WHERE type = ?", data.get("interest_rate_type"))[0]["id"]
        except IndexError:
            interest_rate_type = ""
        # Query database for loans to show
        loans = db.execute(f"""SELECT loans.id, 
        ...
        try:
            payment_frequency = db.execute(
                "SELECT * FROM frequencies WHERE frequency = ?", data.get("payment_frequency"))[0]["id"]
        except IndexError:
            payment_frequency = ""
        # Query database for loans to show
        loans = db.execute(f"""SELECT loans.id, 
...
def bank_filter():
    ...
    if user_bank_id := data.get("user_bank_id"):
        try:
            # Query database for amortizations to show
            banks = db.execute(
                "SELECT * FROM banks WHERE user_id = ? AND user_bank_id = ?", user_id, user_bank_id)
        except IndexError:
            return redirect("/banks")
    # info for filters
    ids = db.execute(
...
def report():
    ...
    if user_loan_id:
        ...
        try:
            ...
            data_amortizations = db.execute(
                """SELECT amortizations.user_amortization_id as id, loans.user_loan_id as loan_id, amortizations.amort_value, amortizations.amort_date FROM amortizations JOIN loans ON amortizations.loan_id = loans.id WHERE loans.user_id = ? AND loans.user_loan_id = ?""", user_id, user_loan_id)
